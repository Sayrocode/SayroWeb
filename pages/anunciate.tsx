// pages/anunciate.tsx (local copy)
...
function Field({ label, children, isRequired }: { label: string; children: React.ReactNode; isRequired?: boolean; }) {
  return (
    <FormControl isRequired={isRequired}>
      <FormLabel mb="1" color="white" fontSize="xs" letterSpacing="0.06em" textTransform="none" fontWeight="semibold">
        {label}
      </FormLabel>
      {children}
    </FormControl>
...
export default function AnunciatePage() {
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      toast({ title: "¡Enviado!", description: "Hemos recibido tu información.", status: "success" });
      e.currentTarget.reset();
    } catch (err: any) {
      toast({ title: "No se pudo enviar", description: err?.message || "Inténtalo de nuevo.", status: "error" });
    } finally {
      setSubmitting(false);
    }
  }
  ...
  return (
    <Layout>
      <Box as="main" bg={GREEN}>
        <Grid templateColumns={{ base: "1fr", md: "minmax(0,1.1fr) minmax(0,0.9fr)" }} minH={{ base: "auto", md: "100vh" }}>
          <GridItem position="relative" bgImage="url(/anunciate.png)" bgSize="cover" bgPos="center" bgRepeat="no-repeat" minH={{ base: "46vh", md: "100%" }} _after={{ content: '""', position: "absolute", inset: 0, bg: "linear-gradient(0deg, rgba(0,0,0,.35) 0%, rgba(0,0,0,.15) 35%, rgba(0,0,0,.15) 100%)", }}>
            <Container maxW="7xl" h="100%" position="relative" zIndex={1}>
              <Box position="absolute" left={{ base: 4, md: 8 }} bottom={{ base: 6, md: 10 }} right={{ base: 4, md: "auto" }} maxW={{ base: "90%", md: "420px" }}>
                <Text color="white" fontWeight="bold" fontSize={{ base: "lg", md: "xl" }} lineHeight={1.35} textShadow="0 2px 14px rgba(0,0,0,.55)">
          ...
          <GridItem bg={GREEN} display="flex" alignItems="center" py={{ base: 8, md: 0 }}>
            <Container maxW="container.sm" px={{ base: 4, md: 8 }}>
              <Box as="section" aria-labelledby="anun-title" bg={GREEN} my={10} color="white" rounded={{ base: "xl", md: "xl" }} boxShadow="0 8px 28px rgba(0,0,0,.35)" border="1px solid" borderColor="whiteAlpha.200" p={{ base: 5, md: 8 }}>
                <Heading id="anun-title" as="h1" fontSize={{ base: "xl", md: "2xl" }} textAlign="center" letterSpacing="0.04em" mb={{ base: 4, md: 5 }}>
                ...
                <Box as="form" onSubmit={onSubmit}>
                  ...
                  <Button type="submit" mt={5} w="full" size="md" color="white" bg="whiteAlpha.300" _hover={{ bg: "whiteAlpha.400" }} border="1px solid" borderColor="whiteAlpha.500" leftIcon={<FiSend />} isLoading={submitting}>
                    Enviar
                  </Button>
